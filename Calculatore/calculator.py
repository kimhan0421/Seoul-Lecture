...
while True:
    # 사용자가 원하는 기능 입력 받기, int()은 추후 기능 조건 확인을 위해 사용 >> input 입력값은 str 형태이므로 int형태로 변경
    action = (input('기능을 입력하세요: '))

    if action not in ['1', '2', '3', '4', '5', 'Q', 'q']:
        continue

    if action == '5' or action == 'Q' or action == 'q':  # 종료 조건 확인
        break  # loop 종료 시키는 함수
